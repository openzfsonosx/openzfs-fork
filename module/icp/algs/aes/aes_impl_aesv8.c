...
extern ASMABI int aes_v8_set_encrypt_key(uint32_t rk[],
    const uint64_t bits, const uint32_t cipherKey[]);
extern ASMABI int aes_v8_set_decrypt_key(uint32_t rk[],
    const uint64_t bits, const uint32_t cipherKey[]);
...
aes_aesv8_generate(aes_key_t *key, const uint32_t *keyarr32, int keybits)
{
	...
	key->nr = aes_v8_set_decrypt_key(keyarr32, keybits,
	    &(key->decr_ks.ks32[0]));
	// asm doesnt return the solaris nrounds yet.
	key->nr = 14;
	kfpu_end();
}
